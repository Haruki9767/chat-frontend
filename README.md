...
- **Room size limits.** Every room has a cap between 2 and 10 people, set when it's created and changeable later by whoever made it.
- **Reply to any message.** Tap a message to quote it in your reply, so context never gets lost in a busy conversation.
- **React with any emoji.** Including reacting to your own messages.
...
3. **Join a room** by entering its number and password (if it has one), or **create a room** and set your own password, name, and size limit.
4. **Chat.** Tap a message to reply to it, tap the 🙂+ icon to react.
5. **Leave anytime** — rejoin later with the same room number and password.
