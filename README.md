...
Paranoid Chat is a private, invite-based real-time chat client for small groups. It provides account authentication, password-protected rooms, optional join tokens, ephemeral rooms, message history, replies, mentions, typing indicators, local themes, and owner-only room management.

**Live application:** <https://paranoid-chats.pages.dev>
**Backend repository:** <https://github.com/Haruki9767/chat>
**Frontend repository:** <https://github.com/Haruki9767/chat-frontend>
...
| Join tokens | Room owners can mint, list, and revoke invitation tokens. A valid token can be used instead of the room password where supported by the backend. |
| Messaging | Connected users can exchange real-time messages over WebSocket. The client supports message history for room types where the backend persists history. |
...
Usernames and passwords are validated by the backend. There is no password-reset flow in the current client, so users must retain their credentials securely.
...
The **E2EE (in development)** option is intentionally disabled. Do not treat the current E2EE protocol messages or server-side room support as a usable encrypted-chat feature.
...
Select **Leave** to close the current WebSocket connection and return to the room screen. Use **Log out** to end the authenticated session.

## Configuration

The frontend uses the backend URL exposed by the Cloudflare Pages Function at `/api/config` through `functions/config.js`. The function reads the `BACKEND_URL` environment variable and emits:

```js
window.BACKEND_URL = "https://your-backend.example";
```

If the environment variable is not set, the current client falls back to:

```text
https://chat.lime-paranoid.workers.dev
```
...
For local preview, serve the repository directory with any static HTTP server. When using Cloudflare Pages, deploy the repository as a static site with the `functions/` directory enabled so `/api/config` can read `BACKEND_URL`.

The backend must be running and reachable at the configured URL. The frontend expects the backend to provide the authentication, room, WebSocket, room-history, password-management, and join-token endpoints used by `script.js`.

## Security and privacy notes

The application is designed for trusted groups and does not provide public room discovery. Use only backend servers you trust when configuring or deploying the client. The client sends authentication and room requests to the configured backend URL, so that endpoint controls how those requests are handled.
...
The frontend escapes message content before rendering it and opens detected links in a new tab with `noopener`, `noreferrer`, and `nofollow`. These client-side protections do not replace secure backend authentication, authorization, rate limiting, input validation, and transport security.
...
| `script.js` | Client state, authentication requests, WebSocket lifecycle, room actions, message rendering, settings, and API integration. |
| `style.css` | Visual theme, layout, responsive behavior, and component styling. |
| `functions/config.js` | Cloudflare Pages Function that exposes the configured backend URL to the browser. |
| `_headers` | Cloudflare Pages security and CORS response headers. |

## Important limitations

The current frontend does not implement password recovery, public room discovery, client-side E2EE, or offline message delivery. A room connection depends on the configured backend and an active network connection.

## References

[1]: https://paranoid-chats.pages.dev "Paranoid Chat live application"
[2]: https://github.com/Haruki9767/chat "Paranoid Chat backend repository"
[3]: https://github.com/Haruki9767/chat-frontend "Paranoid Chat frontend repository"
[4]: https://developers.cloudflare.com/pages/functions/ "Cloudflare Pages Functions documentation"
[5]: https://www.hcaptcha.com/ "hCaptcha"
